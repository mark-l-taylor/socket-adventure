...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def route(self):
        ...
        # My logic
        if self.input_buffer.startswith("quit"):
            self.quit(None)
        elif self.input_buffer.lower().startswith('move'):
            self.move(self.input_buffer.lower().split('move ')[-1].strip())
        elif self.input_buffer.lower().startswith('say '):
            self.say(self.input_buffer.lower().split('say ')[-1].strip())
        else:
            pass

        # Activity answer logic
